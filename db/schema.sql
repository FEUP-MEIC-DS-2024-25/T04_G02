...
CREATE
OR REPLACE FUNCTION increment_version_requirements() RETURNS TRIGGER AS $ $ BEGIN NEW.version := COALESCE(
  (
    SELECT
      MAX(version) + 1
    FROM
      req_to_story.requirementshistory
    WHERE
...
.CREATE TRIGGER increment_version_trigger_requirements BEFORE
INSERT
  ON RequirementsHistory FOR EACH ROW EXECUTE FUNCTION increment_version_requirements();

CREATE
OR REPLACE FUNCTION increment_version_userstory() RETURNS TRIGGER AS $ $ BEGIN NEW.version := COALESCE(
  (
    SELECT
      MAX(version) + 1
    FROM
      req_to_story.userstoryhistory
    WHERE
      userstory_id = NEW.userstory_id
  ),
  1
);
...
CREATE TRIGGER increment_version_trigger_userstory BEFORE
INSERT
  ON UserStoryHistory FOR EACH ROW EXECUTE FUNCTION increment_version_userstory();